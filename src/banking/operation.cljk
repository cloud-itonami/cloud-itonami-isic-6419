(ns banking.operation
  "OperationActor -- one banking operation = one supervised actor run,
  expressed as a langgraph-clj StateGraph. The advisor (BankingOps-
  LLM) is sealed into a single node (:advise); its proposal is ALWAYS
  routed through the Monetary Intermediation Governor (:govern) and
  ...
  One graph run = one banking operation (intake -> advise -> govern ->
  ...
  decision to a human operator (banking operator). The approver
  resumes with `{:approval {:status :approved}}` (or :rejected).
  `:actuation/post-settlement`/`:actuation/dispatch-interbank-message`
  ALWAYS reach this node when the governor is clean -- see `banking.
  phase`."
...
(defn build
  ...
  `banking.store/Store`).
  opts:
    :advisor      -- a `banking.bankingadvisor/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (bankingadvisor/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; BankingOps-LLM inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (bankingadvisor/-advise advisor store request)]
            {:proposal p :audit [(bankingadvisor/trace request p)]})))

      ;; Monetary Intermediation Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
